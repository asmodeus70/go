...
var (
	...
	tagPrefix      = flag.String("tagPrefix", "mongo-snap", "String to prefix to tag name, description")
...
func CreateSnapshotTags(svc *ec2.EC2, resourceID, volumeName, volumeID string) error {
	...
	if volumeName == "" {
		nVal = fmt.Sprintf("%s: %s, %s", *tagPrefix, volumeID, time.Now().Format("02-01-2017"))
	} else {
		nVal = fmt.Sprintf("%s: %s, %s", *tagPrefix, volumeName, time.Now().Format("02-01-2017"))
	}
